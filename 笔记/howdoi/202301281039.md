...
`howdoi` 是一个命令行工具，用来查询一些常用的基础知识，起到备忘录快速查询的作用，命令行输入的查询语言为英文，当然如果使用其他语言的话也并非会使程序无法运行，只是能找到恰当答案的微乎其微。例如你在写代码的时候忘记了Python列表的切片方法，有了 `howdoi` 命令行工具你就可以这样去做:
```
